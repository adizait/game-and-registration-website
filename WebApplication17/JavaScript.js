...
function username() {
    ...
    if (username.length == 0) {
        document.getElementById("usernameError").innerHTML = "חובה להכניס שם משתמש";
        return false;
    }
    for (var i = 0; i < username.length; i++) {
        for (var j = 0; j < str1.length; j++) {
            if (username[i] == str1[j]) {
                document.getElementById("usernameError").innerHTML = "שם המשתמש חייב להכיל אותיות באנגלית ומספרים בלבד";
                return false;
            }
        }
    }
...
function allFunction() {
    ...
    var chkG = gender();
    var chkBD = birthday();
    return chkFN && chkLN && chkUS && chkPS && chkE && chkP && chkAD && chkG && chkBD;
}